    ...
    export default{
        ...
        methods: {
            getOptionsList:function(){
                var that = this;
                tools.ajax({
                    url: '/api/admin/get/sms/type',
                    ajaxData: {},
                    successFun: function (res) {
                        that.options = res.list;

                        if(tools.url.params('type')){
                            for(var i = 0; i < that.options.length; i++){
                                if(tools.url.params('type') == that.options[i].key){
                                    that.form.type = that.options[i];
                                }
                            }
                        }else {
                            that.form.type = '';
            ...
            getList : function(page,page_size){
                ...
                if(that.is_init){
                    if(tools.url.params('content')){
                        ajaxData.content = decodeURI(tools.url.params('content'));
                    }
                    if(tools.url.params('type')){
                        ajaxData.type = decodeURI(tools.url.params('type'));
                    }
            ...
            search: function () {
                var that = this;
                if(that.form.type != '' && that.form.type !== null){
                    that.extra_data.type = that.form.type.key;
                }else {
                    delete that.extra_data.type;
                }
        ...
        watch: {
            extra_data: {
                handler: function (data, oldVal) {
                    ...
                    if(data.type){
                        that.extra_data.type = that.form.type;
                    }else {
                        delete that.extra_data.type;
                    }
