<template>
    <div i="power_lists">
        <div class="mb10">
            <button class="layui-btn" @click="go('/admin/power/edit')">添加权限组</button>
        </div>
        <base-table
                :title="title"
                :tableData="lists"
    ...
    export default{
        ...
        methods: {
            ...
            edit: function (index, value) {
                tools.router.push({path: '/admin/power/edit', query: {id: this.lists[index].id}});
            },

            /* 权限管理 */
            goAdmin: function (index, value) {
                var that = this;
                that.go('/admin/power/admin/lists',{'id':that.lists[index].id});
            },
